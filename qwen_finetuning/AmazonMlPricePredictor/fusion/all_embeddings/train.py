"""
Training script for 4-embedding fusion model
"""
import sys
import numpy as np
import pandas as pd
...
from .dataset import FourEmbeddingDataset, create_data_splits
from .model import FourBranchFusionModel, AdvancedMoEFusionModel, count_parameters
...
def main():
    ...
    print("="*60)
    print("FOUR-EMBEDDING FUSION MODEL TRAINING")
    print("="*60)
    print(f"Device: {device}")
    print(f"CLIP image dim: {config.CLIP_IMAGE_DIM}")
    print(f"SigLIP image dim: {config.SIGLIP_IMAGE_DIM}")
    print(f"SigLIP text dim: {config.SIGLIP_TEXT_DIM}")
    print(f"Qwen text dim: {config.QWEN_TEXT_DIM}")
